...
function PhotoCard(props) {
  let allReviews = () => {
    if (props.profile_info.reviews === undefined) {
      ...
      props.profile_info.reviews.length &&
      props.profile_info.reviews.length < 1
    ) {
      return 0;
    } else {
  ...
  let allBucketlistItems = () => {
    if (props.profile_info.bucketlist === undefined) {
      ...
      props.profile_info.bucketlist.length &&
      props.profile_info.bucketlist.length < 1
    ) {
      return 0;
    } else {
  ...
  let { username, bio } = props.profile_info;
    ...
    <Segment className="photo-container">
      <div className="photo-header-container">
        ...
        <div className="photo-bucketlist-container">
          ...
          <span className="photo-bucketlist-number">
            {allBucketlistItems()}
          </span>
        </div>
      </div>
